...
// Permite que o servidor carregue qualquer conteúdo externo (necessário para o iframe funcionar)
...
const api = axios.create({
    ...
    headers: {
        ...
        'Referer': 'https://www.pobreflixtv.uk/' 
    },
    timeout: 15000 
});
...
app.get('/', (req, res) => {
    res.json({ status: "Online", msg: "API Anti-Redirect Ativo" });
});
...
app.get('/v1/get/recommeds', async (req, res) => {
    try {
        ...
        const $ = cheerio.load(response.data);
        const data = { movies: { releases: [], trending: [] }, series: { releases: [], trending: [] } };
        ...
        res.json(data);
    } catch (error) { res.status(500).json({ error: "Erro ao carregar home" }); }
});
...
app.get('/v1/info', async (req, res) => {
    ...
    try {
        ...
        const result = {
            id: extractId(url), title, is_series: isSeries, year, imdb, thumb, description: desc, episodes: [], watch_link: null 
        };
        ...
        if (isSeries) {
            ...
            $('#listagem li').each((index, element) => {
                ...
                if (epUrl) {
                    episodes.push({ name: epName, player_id: extractId(epUrl), url: epUrl, order: sortId });
                }
            });
        ...
        res.json(result);
    } catch (error) { res.status(500).json({ error: "Erro ao pegar detalhes" }); }
});

// --- ROTA DE ASSISTIR BLINDADA ---
app.get('/v1/watch/:id', (req, res) => {
    ...
    // A MÁGICA ESTÁ AQUI:
    // 1. sandbox: Permite scripts e mesma origem, mas NÃO permite 'allow-top-navigation' (redirecionar a pagina) e 'allow-popups' (abrir abas).
    // 2. CSS pointer-events: Alguns ads criam camadas invisíveis. O Iframe ocupa 100% mas garantimos que não tem margem.
    ...
    <head>
        ...
        <!-- CSP Interna: Permite carregar iframes de qualquer lugar, mas bloqueia formulários e objetos estranhos -->
        <meta http-equiv="Content-Security-Policy" content="default-src * 'unsafe-inline' 'unsafe-eval' data: blob:; frame-src *; navigate-to 'none';">
